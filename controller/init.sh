...
##sed -i '/\connection = sqlite:////var/lib/neutron/neutron.sqlite/c connection = mysql+pymysql://neutron:neutron@controller/neutron' /etc/neutron/neutron.conf
##sed -i "765i connection = mysql+pymysql://neutron:neutron@controller/neutron" /etc/neutron/neutron.conf
##[DEFAULT]
...
##auth_type = password was global in sed [keystone_authtoken]
...
##NO FUNCIONA!
sed -i "s|#ALLOWED_HOSTS = ['horizon.example.com', ]|ALLOWED_HOSTS = ['one.example.com', 'two.example.com']|" /etc/openstack-dashboard/local_settings.py
